...
can be used to test the LDAP integration of Bulma.
...
## Enabling Integration in Bulma

After making sure that the local OpenLDAP server works, Bulma will be able to connect to it.

For this you will need to go to the Backend > Settings > Integrations > LDAP and enable the integration from there. 

This settings page also requires some configuration for Bulma to be able to connect to the server and pull
...
If the filter was right, you will be getting LDAP entries displayed on screen that can be imported into Bulma.
...
While SSO with LDAP on Bulma, the username will need to be matched for the SSO to work. Also, the LDAP DN 
...
If LDAP is enabled, Bulma will first try to check for the local credentials, but if this does not work, then 
