...
the CalDAV integration of Bulma.
...
- The right time zone value needs to be selected for synchronization to match the information Bulma sends.
...
After making sure that the local Baikal server works, Bulma will be able to connect to it and sync the
appointment data.
...
While trying to enable the CalDAV sync from the Bulma calendar page, use the following credentials after
