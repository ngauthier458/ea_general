...
class Google extends EA_Controller
{
    ...
    /**
     * Complete synchronization of appointments between Google Calendar and Bulma.
     *
    ...
    public static function sync(?string $provider_id = null): void
    {
        try {
            ...
            foreach ($local_events as $local_event) {
                ...
                try {
                    ...
                    if ($google_event->getStatus() == 'cancelled') {
                        throw new Exception('Event is cancelled, remove the record from Bulma.');
                    }

                    // If Google Calendar event is different from Bulma appointment then update Bulma record.
                    ...
                    // Appointment not found on Google Calendar, delete from Bulma.
            ...
            // Add Google Calendar events that do not exist in Bulma.
            ...
            foreach ($google_events->getItems() as $google_event) {
                ...
                // Record doesn't exist in the Bulma, so add the event now.
                $local_event = [
     ...
     * documentation of OAuth), every Bulma installation should use its own calendar api key. So in every
     * api console account, the "http://path-to-Bulma/google/oauth_callback" should be included in an
